...
export const Hero = () => {
  ...
  const handleScroll = (e: React.MouseEvent<HTMLAnchorElement>, targetId: string) => {
    ...
    if (elem) {
        // Sprawdzamy szerokość okna bezpośrednio przy kliknięciu (najbezpieczniej)
  ...
  // Standardowe ustawienia animacji (użyjemy CSS żeby je wyłączyć na mobile)
  const fadeInUp = {
  ...
  return (
    ...
    <style jsx global>{`
      ...
      /* FIX NA WYDAJNOŚĆ I BRAK ANIMACJI NA MOBILE:
         Wymuszamy widoczność elementów na ekranach < 1024px.
         Nadpisuje to 'initial={{opacity: 0}}' z Framer Motion zanim JS ruszy.
         Dzięki temu nie ma mignięcia ani przesunięcia (CLS = 0).
      */
    ...
    <section 
        // USUNIĘTO: content-visibility-auto (powoduje problemy z CLS na górze strony)
    ...
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden" aria-hidden="true">
        {/* Tło mobilne statyczne */}
        <div className="lg:hidden absolute inset-0 bg-[radial-gradient(circle_at_50%_40%,rgba(37,99,235,0.12),transparent_60%)]"></div>

        {/* Tło desktopowe animowane - renderujemy warunkowo (isMounted), bo to ciężkie dla GPU, 
            ale ponieważ jest 'absolute', nie wpłynie na CLS. */}
        {isMounted && (
      ...
      <div className="w-full max-w-[1800px] px-6 md:px-12 relative z-10 flex flex-col lg:flex-row items-center gap-16 lg:gap-32">
        ...
        <div className="flex-1 text-center lg:text-left space-y-8 lg:space-y-10 relative z-20">
          ...
          <motion.div 
            {...fadeInUp}
            // Dodajemy klasę mobile-no-motion, żeby na telefonie było statyczne od razu
            className="mobile-no-motion inline-flex items-center gap-2 px-5 py-2 rounded-full bg-slate-900/80 border border-slate-800 text-slate-300 text-xs font-semibold tracking-wide backdrop-blur-sm mx-auto lg:mx-0"
          >
          ...
          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold tracking-tight leading-[1.05]">
            <motion.span
                ...
                className="mobile-no-motion block"
            >
                TWOJA FIRMA <br />
            </motion.span>
            <motion.span
          ...
          <motion.p 
             ...
             className="mobile-no-motion text-lg md:text-xl text-slate-400 max-w-2xl mx-auto lg:mx-0 leading-relaxed"
          >
          ...
          <motion.div 
            ...
            className="mobile-no-motion flex flex-col sm:flex-row gap-5 justify-center lg:justify-start pt-4 lg:pt-6"
          >
            <Link 
              href="/#oferta"
        ...
        {/* ZAWSZE w DOM-ie, ale ukryta CSS-em. To klucz do zerowego CLS. */}
        <div className="hidden lg:block flex-1 w-full max-w-[650px] relative perspective-1000">
            ...
            >
                {/* Bloby - tylko jeśli zamontowane (bo ciężkie), ale są absolute więc nie psują layoutu */}
                {isMounted && (
