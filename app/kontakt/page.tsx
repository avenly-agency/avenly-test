            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                >
                    ...
                    <p className="text-sm sm:text-base md:text-lg text-slate-400 max-w-xl mx-auto leading-relaxed">
                        Masz pytania czy gotowy plan? Niezależnie od etapu, chętnie doradzimy i zamienimy Twoją wizję w działający produkt. 
                        Skontaktuj się z nami – bez handlowego lania wody, tylko konkretne rozwiązania, które realnie rozwiną Twój biznes.
                    </p>
                </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
                    ...
                    >
                        <div className="space-y-6">
                            ...
                            <InfoCard 
                                ...
                                sub="Pracujemy zdalnie globalnie."
                            />
                        </div>
                    ...
                    <motion.div 
                        ...
                        className="relative"
                    >
                        <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/20 to-indigo-500/20 rounded-[2rem] blur-xl opacity-50" aria-hidden="true"></div>
                        
                        {/* PADDING NA MOBILE: p-5 */}
                                ...
                                {isSuccess ? (
                                    ...
                                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                        ...
                                        <div className="space-y-2">
                                            {/* ZMIANA: Dodano mb-2 block dla większego odstępu */}
                                            <label htmlFor="message" className="text-sm font-bold text-slate-300 ml-1 mb-2 block">Wiadomość</label>
                                            <textarea 
                                                id="message"
                                                rows={5}
                                                className={cn(
                                                ...
                                                {...register("message", { required: "Wiadomość nie może być pusta", minLength: { value: 10, message: "Minimum 10 znaków" } })}
                                            ></textarea>
                                            {errors.message && <span className="text-xs text-red-500 ml-1 flex items-center gap-1"><AlertCircle size={10} /> {errors.message.message}</span>}
                                        </div>
                                        ...
                                        <div className="pt-2">
                                            <label className="flex items-start gap-3 cursor-pointer group">
                                                ...
                                                <span className="text-xs text-slate-400 leading-relaxed group-hover:text-slate-300 transition-colors">
                                                    Wyrażam zgodę na przetwarzanie moich danych osobowych zgodnie z <Link href="/polityka-prywatnosci" className="text-blue-400 hover:underline">Polityką Prywatności</Link>. Rozumiem, że mogę wycofać zgodę w każdej chwili.
                                                </span>
                                            </label>
                                            {errors.privacy && <span className="text-xs text-red-500 mt-2 block ml-1">{errors.privacy.message}</span>}
                                        </div>
...
// ZMIANA: space-y-2 (trzyma input i błąd blisko)
// ZMIANA: mb-2 block dla Labela (odpycha label od inputa)
const InputGroup = ({ label, id, error, type = "text", ...props }: any) => (
    <div className="space-y-2">
        <label htmlFor={id} className="text-sm font-bold text-slate-300 ml-1 mb-2 block">{label}</label>
        <input 
            id={id}
            type={type}
            className={cn(
            ...
            {...props}
        />
        {error && <span className="text-xs text-red-500 ml-1 flex items-center gap-1"><AlertCircle size={10} /> {error.message}</span>}
    </div>
);
...
const InfoCard = ({ icon: Icon, title, value, sub, href }: any) => (
    <div className="flex items-start gap-5 group">
        ...
        <div>
            ...
            {href ? (
                <a href={href} className="text-xl md:text-2xl font-bold text-white hover:text-blue-400 transition-colors">{value}</a>
            ) : (
