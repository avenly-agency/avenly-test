...
export default function ContactPage() {
	...
	const onSubmit = async (data: FormData) => {
		...
		// Jeśli botcheck jest zaznaczony (przez bota), nie wysyłamy
		if (data.botcheck) {
		...
		try {
			// 👇 WSTAW TUTAJ SWÓJ KLUCZ Z WEB3FORMS 👇
			...
			<div className="container mx-auto px-6 relative z-10">
				...
				<motion.div
					...
					<p className="text-sm sm:text-base md:text-lg text-slate-400 max-w-xl mx-auto leading-relaxed">
						Masz pytania czy gotowy plan? Niezależnie od etapu, chętnie doradzimy i zamienimy Twoją wizję w działający
						produkt. Skontaktuj się z nami – bez handlowego lania wody, tylko konkretne rozwiązania, które realnie
						rozwiną Twój biznes.
					</p>
				</motion.div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
					...
					<motion.div
						...
						<div className="space-y-6">
							...
							<InfoCard
								icon={Phone}
								title="Telefon"
								value="+48 668 124 367"
								href="tel:+48668124367"
								sub="Pon-Pt, 9:00 - 17:00"
							/>
							<InfoCard icon={MapPin} title="Biuro" value="Polska" sub="Pracujemy zdalnie globalnie." />
						</div>
						...
						<div className="flex gap-4">
							<SocialButton icon={Facebook} href="https://www.facebook.com/profile.php?id=61581862509345" />
							<SocialButton icon={Instagram} href="https://www.instagram.com/avenly.pl/" />
							<SocialButton icon={Github} href="https://github.com/avenly-agency" />
						</div>
					</motion.div>
					...
					<motion.div
						...
						<div className="relative bg-[#080808]/90 backdrop-blur-xl border border-white/10 rounded-[2rem] p-5 sm:p-8 md:p-10 shadow-2xl">
								...
								{isSuccess ? (
									<motion.div
										...
										<button
											...
											className="px-6 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm font-bold transition-colors border border-white/10 cursor-pointer">
											Wyślij kolejną
										</button>
									</motion.div>
								) : (
									<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
										{/* Honeypot field - ukryte */}
										...
										<div className="space-y-2">
											<label htmlFor="message" className="text-sm font-bold text-slate-300 ml-1 mb-2 block">
												Wiadomość
											</label>
											<textarea
												id="message"
										...
										<div className="pt-2">
											{/* ZMIANA: items-start -> items-center */}
											<label className="flex items-center gap-3 cursor-pointer group">
												<div className="relative flex items-center">
													...
													/>
													<div className="w-5 h-5 border border-white/20 rounded bg-[#050505] peer-checked:bg-blue-600 peer-checked:border-blue-600 transition-all flex items-center justify-center shrink-0">
										...
										<button
											...
											// DODANO: cursor-pointer dla pewności
											className="w-full py-4 bg-white text-black font-bold rounded-xl hover:bg-blue-50 transition-all hover:scale-[1.01] active:scale-[0.99] disabled:opacity-70 disabled:pointer-events-none flex items-center justify-center gap-2 shadow-[0_0_20px_-5px_rgba(255,255,255,0.3)] cursor-pointer">
											{isSubmitting ? (
												<>
...
const InputGroup = ({ label, id, error, type = 'text', ...props }: any) => (
	<div className="space-y-2">
		<label htmlFor={id} className="text-sm font-bold text-slate-300 ml-1 mb-2 block">
			{label}
		</label>
		<input
			id={id}
...
const SocialButton = ({ icon: Icon, href }: any) => (
	<a
		...
		className="w-12 h-12 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-slate-400 hover:text-white hover:bg-blue-600 hover:border-transparent transition-all duration-300 cursor-pointer">
		<Icon size={20} />
	</a>
)
